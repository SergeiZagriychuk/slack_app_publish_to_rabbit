...
public abstract class AbstractWorkflow {
	...
	public WorkflowStep buildStep() {
		...
		WorkflowStep step = WorkflowStep.builder().callbackId(getCallbackName()).edit((req, ctx) -> {
			log.info("Edit workflow step '" + getCallbackName() + "' by user "
			...
			if (isRabbitFlow()) {
				...
				if (inputs.containsKey(Keys.routing_key.toString())) {
					p.put("routing_key_init_value", inputs.get(Keys.routing_key.toString()).getValue().toString());
				}
			} else {
				p.put("jenkins_flow", true);
				if (inputs.containsKey(Keys.tenant.toString())) {
					p.put("tenant_init_value", inputs.get(Keys.tenant.toString()).getValue().toString());
				}
			}
			if (inputs.containsKey(Keys.repo.toString())) {
				p.put("repo_init_value", inputs.get(Keys.repo.toString()).getValue().toString());
			}
			if (inputs.containsKey(Keys.suite.toString())) {
				p.put("suite_init_value", inputs.get(Keys.suite.toString()).getValue().toString());
			}
			StringBuilder extraParams = new StringBuilder();
			inputs.keySet().stream().filter(k -> !EnumUtils.isValidEnum(Keys.class, k))
					.forEach(k -> extraParams.append(k + " "));
			if (extraParams.length() != 0) {
				p.put("params_init_value", extraParams.toString());
			}
