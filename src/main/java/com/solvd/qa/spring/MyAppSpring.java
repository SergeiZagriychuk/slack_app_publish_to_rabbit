...
import com.solvd.qa.util.HttpClient;
import com.solvd.qa.util.HttpClient.Response;
import com.solvd.qa.util.Path;
...
public class MyAppSpring {
    ...
    public App initSlackApp() {
        ...
        app = app.service(installationService).service(stateService);

        app.event(AppHomeOpenedEvent.class, (payload, ctx) -> {
            var appHomeView = view(view -> view.type("home").blocks(asBlocks(section(
                    section -> section.text(
                            markdownText(mt -> mt.text("*This is slack app which helps to publish RabbitMQ requests for Jenkins job triggering.*")))),
                    divider(),
                    section(section -> section.text(markdownText(mt -> mt.text(
                            "Such triggering can be added as a step for your custom Workflow in slack")))))));
        ...
        WorkflowStep step = WorkflowStep.builder().callbackId("start_tests")
                ...
                .edit((req, ctx) -> {
                    ...
                    values.put("routing_key_initial_value",
                            inputs.containsKey(Keys.routing_key.toString())
                                    ? String.format(",\"initial_value\" : \"%s\"", inputs.get(Keys.routing_key.toString()).getValue().toString())
                                    : "");
                    values.put("repo_initial_value",
                            inputs.containsKey(Keys.repo.toString())
                                    ? String.format(",\"initial_value\" : \"%s\"", inputs.get(Keys.repo.toString()).getValue().toString())
                                    : "");
                    values.put("suite_initial_value",
                            inputs.containsKey(Keys.suite.toString())
                                    ? String.format(",\"initial_value\" : \"%s\"", inputs.get(Keys.suite.toString()).getValue().toString())
                                    : "");
                    ...
                    ViewsOpenRequest viewsOpenRequest = ViewsOpenRequest.builder().triggerId(triggerId).viewAsString(bodyWInitValues).build();
                ...
                .save((req, ctx) -> {
                    ...
                    // stepOutput(o -> o.name(Keys.routing_key.toString()).type("text").label("RabbitMQ routing key"))
                    // stepOutput(o -> o.name(Keys.repo.toString()).type("text").label("Name of test project repository")),
                    // stepOutput(o -> o.name(Keys.suite.toString()).type("text").label("Test suite to run"))
                    );
                ...
                .execute((req, ctx) -> {
                    ...
                    try {
                        ...
                        String auth = Base64.getEncoder().encodeToString((System.getenv("RABBIT_CREDS")).getBytes("UTF-8"));
                        
//                        RestAssured.given()
                        
                        Response<String> r = HttpClient.uri(Path.EMPTY, System.getenv("RABBIT_PUBLISH_URL"))
                                .withAuthorization("Basic " + auth)
                                .post(String.class, FileUtil.transformJsonToMap(body));
                        Boolean routing = JsonPath.parse(r.getObject()).read("routing");
                        if (routing == null || !routing) {
                            Map<String, Object> error = new HashMap<>();
                            error.put("message", "Error for publishing request to RabbitMQ: " + r.getObject());
                            ctx.fail(error);
                        } else {
        ...
        app.blockAction("job_params_entered", (req, ctx) -> {
            ...
            Map<String, Map<String, ViewState.Value>> stateValues = req.getPayload().getView().getState().getValues();
            String[] params = ((String) stepInput(i -> i.value(extract(stateValues, "custom_params", "job_params_entered"))).getValue()).trim()
                    .split("\\s+");
            ...
            View viewUpd = view(view -> view.type("workflow_step").callbackId("start_tests").blocks(
                    asBlocks(layouts.toArray(new LayoutBlock[layouts.size()]))));
            ViewsUpdateRequest viewsUpdateRequest = ViewsUpdateRequest.builder().viewId(req.getPayload().getView().getId()).view(viewUpd).build();
        ...
        app.endpoint("GET", "/slack/oauth/completion", (req, ctx) -> {
            return com.slack.api.bolt.response.Response.builder()
                    ...
                    .body(SlackAuthSuccessController.renderCompletionPageHtml(req.getQueryString()))
                    .build();
        });
        ...
        app.endpoint("GET", "/slack/oauth/cancellation", (req, ctx) -> {
            return com.slack.api.bolt.response.Response.builder()
                    .statusCode(200)
                    .contentType("text/html")
                    .body(SlackAuthCancelController.renderCancellationPageHtml(req.getQueryString()))
                    .build();
        });
    ...
    private static String extract(Map<String, Map<String, ViewState.Value>> stateValues, String blockId, String actionId) {
